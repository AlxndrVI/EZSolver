...
int EZSolver3(int m[mxmy][mxmy], int solution[mxmy][mxmy])
{
	int nm[mxmymxmy];
	...
	for (int b = 0; b < mxmy * 3; ++b)
	{
		...
		blockOneOrMore[0][b] = 0;
		blockOneOrMore[1][b] = 0;
	}
	...
	for (int o = 0; o < mxmymxmy; ++o)
	{
		...
		if (c)
		{
			...
			blockOneOrMore[0][offset2row[o]] |= c;
			blockOneOrMore[0][offset2column[o]] |= c;
			blockOneOrMore[0][offset2block[o]] |= c;
		}
		else
		{
	...
	while (queuelenght && (ql != queuelenght))
	{
		...
		for (int q = 0; q < ql; ++q)
		{
			int ic = blockOneOrMore[curr][offset2row[queue[q]]] | blockOneOrMore[curr][offset2column[queue[q]]] | blockOneOrMore[curr][offset2block[queue[q]]];
			int c = ic^mflags;
			...
			{
				nm[queue[q]] = d;
				blockOneOrMore[curr][offset2row[queue[q]]] |= d;
				blockOneOrMore[curr][offset2column[queue[q]]] |= d;
				blockOneOrMore[curr][offset2block[queue[q]]] |= d;
				blockOneOrMore[next][offset2row[queue[q]]] |= d;
				blockOneOrMore[next][offset2column[queue[q]]] |= d;
				blockOneOrMore[next][offset2block[queue[q]]] |= d;
			}
			else if (bitcountarray[c] == 1) //если 1 вариант
			{
				nm[queue[q]] = c;
				blockOneOrMore[curr][offset2row[queue[q]]] |= c;
				blockOneOrMore[curr][offset2column[queue[q]]] |= c;
				blockOneOrMore[curr][offset2block[queue[q]]] |= c;
				blockOneOrMore[next][offset2row[queue[q]]] |= c;
				blockOneOrMore[next][offset2column[queue[q]]] |= c;
				blockOneOrMore[next][offset2block[queue[q]]] |= c;
			}
